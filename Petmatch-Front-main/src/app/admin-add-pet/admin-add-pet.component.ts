...
@Component({
  selector: 'app-admin-add-pet',
  templateUrl: './admin-add-pet.component.html',
  styleUrls: ['./admin-add-pet.component.css']
})
export class AdminAddPetComponent {
  pet = new Pet();
  ...
  addPet() {
    ...
    this.petService.createPet(formData).subscribe(
      (response) => {
        ...
        this.router.navigate(['/admin-petmatch-pets']); // Redirect to the pet list or profile page
      },
      (error) => {
